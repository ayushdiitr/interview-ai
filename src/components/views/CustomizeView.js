...
export class CustomizeView extends LitElement {
    ...
    static properties = {
        ...
        examGroqAutoCapture: { type: Boolean },
    };
    ...
    constructor() {
        ...
        // Font transparency default
        this.fontTransparency = 1.0;
        ...
        this.selectedGroqModel = 'gpt-oss-120b';

        // Exam Groq auto-capture default (disabled by default)
        this.examGroqAutoCapture = false;

        // Show Analyze Button default (enabled by default)
        ...
        this.loadFontTransparency();
        this.loadFontSize();
        this.loadModeSettings();
        this.loadExamGroqAutoCapture();
        this.loadShowAnalyzeButton();
    ...
    getProfiles() {
        return [
            ...
            {
                value: 'product',
                name: 'Job Interview - Product Based',
                description: 'Product sense, estimation, strategy & metrics for PM interviews at top companies',
            },
            {
                value: 'sales',
                name: 'Sales Call',
            ...
            {
                value: 'exam-groq',
                name: 'Exam Assistant (Groq)',
                description: 'NEW & RECOMMENDED: Exam assistance powered by Groq API - faster responses',
            },
            {
                value: 'exam',
                name: 'Exam Assistant (Gemini)',
                description: 'Academic assistance using Gemini API for test-taking and exam questions',
            },
        ];
    }
    ...
    getProfileNames() {
        return {
            ...
            'exam-groq': 'Exam Assistant (Groq)',
            exam: 'Exam Assistant (Gemini)',
    ...
    getDefaultKeybinds() {
        ...
        return {
            ...
            latestResponse: isMac ? 'Cmd+Shift+]' : 'Ctrl+Shift+]',
    ...
    getKeybindActions() {
        return [
            ...
            },
            {
                ...
                description: 'Jump directly to the latest AI response in the session',
            },
            {
                key: 'scrollUp',
    ...
    updateBackgroundTransparency() {
        ...
        root.style.setProperty('--modal-overlay-background', `rgba(0, 0, 0, ${this.backgroundTransparency * 0.7})`);
    }

    loadFontTransparency() {
        const fontTransparency = localStorage.getItem('fontTransparency');
        if (fontTransparency !== null) {
            this.fontTransparency = parseFloat(fontTransparency) || 1.0;
        }
        this.updateFontTransparency();
    }

    handleFontTransparencyChange(e) {
        this.fontTransparency = parseFloat(e.target.value);
        localStorage.setItem('fontTransparency', this.fontTransparency.toString());
        this.updateFontTransparency();
        this.requestUpdate();
    }
    ...
    updateFontTransparency() {
        const root = document.documentElement;
        root.style.setProperty('--text-color', `rgba(229, 229, 231, ${this.fontTransparency})`);
        root.style.setProperty('--description-color', `rgba(255, 255, 255, ${this.fontTransparency * 0.6})`);
        root.style.setProperty('--option-label-color', `rgba(255, 255, 255, ${this.fontTransparency * 0.8})`);
        root.style.setProperty('--header-actions-color', `rgba(255, 255, 255, ${this.fontTransparency * 0.6})`);
        root.style.setProperty('--label-color', `rgba(255, 255, 255, ${this.fontTransparency * 0.9})`);
        root.style.setProperty('--placeholder-color', `rgba(255, 255, 255, ${this.fontTransparency * 0.4})`);
        root.style.setProperty('--icon-button-color', `rgba(229, 229, 231, ${this.fontTransparency})`);
        root.style.setProperty('--screen-option-text', `rgba(255, 255, 255, ${this.fontTransparency * 0.7})`);
        root.style.setProperty('--link-color', `rgba(0, 122, 255, ${this.fontTransparency})`);

        // Specific component overrides for transparency
        root.style.setProperty('--model-badge-interview-color', `rgba(52, 211, 153, ${this.fontTransparency})`);
        root.style.setProperty('--model-badge-coding-color', `rgba(251, 146, 60, ${this.fontTransparency})`);
        root.style.setProperty('--analyze-btn-text-color', `rgba(167, 139, 250, ${this.fontTransparency})`);
        root.style.setProperty('--start-button-color', `rgba(0, 0, 0, ${this.fontTransparency})`);
    }
    ...
    loadModeSettings() {
        ...
        let selectedGroqModel = localStorage.getItem('selectedGroqModel');

        // Migrate deprecated model keys
        const DEPRECATED_MODEL_MAP = { 'llama-4-maverick': 'gpt-oss-120b', 'llama-4-scout': 'gpt-oss-20b' };
        if (selectedGroqModel && DEPRECATED_MODEL_MAP[selectedGroqModel]) {
        ...
        this.selectedGroqModel = selectedGroqModel || 'gpt-oss-120b';
    }

    loadExamGroqAutoCapture() {
        const saved = localStorage.getItem('examGroqAutoCapture');
        this.examGroqAutoCapture = saved === 'true';
    }

    handleExamGroqAutoCaptureChange(e) {
        this.examGroqAutoCapture = e.target.checked;
        localStorage.setItem('examGroqAutoCapture', this.examGroqAutoCapture.toString());
        this.requestUpdate();
    }
    ...
    loadShowAnalyzeButton() {
        const saved = localStorage.getItem('showAnalyzeButton');
        if (saved !== null) {
            this.showAnalyzeButton = saved === 'true';
        }
    }

    handleShowAnalyzeButtonChange(e) {
        this.showAnalyzeButton = e.target.checked;
        localStorage.setItem('showAnalyzeButton', this.showAnalyzeButton.toString());
        this.requestUpdate();
    }
            ...
            <div class="settings-container">
                ...
                ? html`
                                      ...
                                      <div class="form-row">
                                          <div class="form-group">
                                              ...
                                              <div class="mode-display-box">💻 Coding/OA Mode (Screenshot-based)</div>
                                              <div class="form-description">
                                                  ...
                                                  .options=${[
                        {
                            ...
                            label: 'Gemini 2.5 Flash (Faster, Balanced)',
                            icon: './assets/models/500px-Google_Gemini_icon_2025.svg.png',
                        },
                        {
                            value: 'gemini-2.5-pro',
                            label: 'Gemini 2.5 Pro (Slower, More Accurate)',
                            icon: './assets/models/500px-Google_Gemini_icon_2025.svg.png',
                        },
                    ]}
                ...
                : html`
                                      ...
                                      <div class="form-row">
                                          <div class="form-group">
                                              ...
                                              <div class="mode-display-box">🎤 Interview Mode (Real-time Audio)</div>
                                              <div class="form-description">
                                                  ${this.getProfileNames()[this.selectedProfile]} profile uses Groq Whisper for speech-to-text and
                                                  GPT OSS for response generation.
                                              </div>
                                          </div>
                                      </div>
                        ...
                        {
                            value: 'gpt-oss-120b',
                            label: 'GPT OSS 120B (Accurate, Powerful)',
                            icon: './assets/models/openai_logo.png',
                        },
                        {
                            value: 'gpt-oss-20b',
                            label: 'GPT OSS 20B (Faster, Lightweight)',
                            icon: './assets/models/openai_logo.png',
                        },
                    ]}
                                              ...
                                              <div class="form-description">
                                                  ${this.selectedGroqModel === 'gpt-oss-120b'
                        ? 'GPT OSS 120B: 120B parameters MoE model. More accurate and detailed responses for complex questions.'
                        : 'GPT OSS 20B: 20B parameters MoE model. Faster responses, ideal for quick interactions.'}
                                              </div>
                                          </div>
                                      </div>

                                      ${this.selectedProfile === 'exam-groq'
                        ? html`
                                                <!-- Auto-capture toggle for Exam Assistant (Groq) -->
                                                <div class="form-group full-width" style="margin-top: 12px;">
                                                    <div class="checkbox-group">
                                                        <input
                                                            type="checkbox"
                                                            class="checkbox-input"
                                                            id="exam-groq-auto-capture"
                                                            .checked=${this.examGroqAutoCapture}
                                                            @change=${this.handleExamGroqAutoCaptureChange}
                                                        />
                                                        <label for="exam-groq-auto-capture" class="checkbox-label">
                                                            🔄 Auto-Capture Screen (Every 10 seconds)
                                                        </label>
                                                    </div>
                                                    <div class="form-description">
                                                        ${this.examGroqAutoCapture
                                ? html`<strong style="color: #34d399;">Enabled:</strong> The app will automatically
                                                                  capture and analyze your screen every 10 seconds. Great for proctored exams where
                                                                  you can't manually trigger captures.`
                                : html`<strong>Disabled:</strong> Manual capture only. Press
                                                                  <strong>Ctrl+Enter</strong> to analyze screen on demand.`}
                                                    </div>
                                                </div>
                                            `
                        : ''}
                ...
                <div class="settings-section">
                    ...
                    <div class="form-grid">
                        ...
                        ${this.selectedProfile === 'interview' && this.vadEnabled
                ? html`
                                      <div class="form-row">
                                          <div class="form-group">
                                              <label class="form-label">
                                                  ...
                                                  <span class="current-selection"
                                                      >${this.vadMode === 'automatic' ? 'Smart Detection' : 'Manual Control'}</span
                                                  >
                                              </label>
                                              <custom-dropdown
                                                  .value=${this.vadMode}
                        ...
                        ? html`<strong>Smart Detection:</strong> VAD automatically detects speech and silence. Best for
                                                        natural conversations where the interviewer speaks continuously.`
                        : html`<strong>Manual Control:</strong> Click the microphone button to start listening. VAD captures
                                                        the entire question (including pauses) until you click again to generate the response. Perfect
                                                        for interviewers who pause frequently mid-sentence.`}
                                          </div>
                                      </div>
                                  `
                : ''
                ...
                <div class="settings-section">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group full-width">
                            <div class="slider-container">
                                <div class="slider-header">
                                    <label class="form-label">Font Transparency</label>
                                    <span class="slider-value">${Math.round(this.fontTransparency * 100)}%</span>
                                </div>
                                <input
                                    type="range"
                                    class="slider-input"
                                    ...
                                    max="1"
                                    step="0.01"
                                    .value=${this.fontTransparency}
                                    @input=${this.handleFontTransparencyChange}
                                />
                                <div class="slider-labels">
                                    <span>Transparent</span>
                                    <span>Opaque</span>
                                ...
                                <div class="form-description">
                                    Adjust the transparency of text elements
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="form-group full-width">
                                <div class="checkbox-group">
                                    <input
                                        type="checkbox"
                                        class="checkbox-input"
                                        id="show-analyze-button"
                                        .checked=${this.showAnalyzeButton}
                                        @change=${this.handleShowAnalyzeButtonChange}
                                    />
                                    <label for="show-analyze-button" class="checkbox-label">
                                        Show "Analyze Screen & Answer" Button
                                    </label>
                                </div>
                                <div class="form-description" style="margin-left: 24px; margin-top: -8px;">
                                    Toggle the visibility of the screen analysis button in the assistant view.
                                </div>
                            </div>
                        </div>
