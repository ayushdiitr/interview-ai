                ...
                async sendRealtimeInput(input) {
                    ...
                    try {
                        ...
                        if (input.media || input.text) {
                            ...
                            const contents = [...this.conversationHistory, { role: 'user', parts: parts }];
                            ...
                            try {
                                ...
                                if (responseText && responseText.trim()) {
                                    ...
                                    this.conversationHistory.push({ role: 'user', parts: parts }, { role: 'model', parts: [{ text: responseText }] });
                        ...
                        if (error.message && error.message.toLowerCase().includes('429')) {
                            ...
                            // Look for "Please retry in Xs" or similar patterns in error
                            const retryMatch = error.message.match(/retry in (\d+(?:\.\d+)?)\s*s/i);
                            if (retryMatch) {
                                retryAfterSeconds = Math.ceil(parseFloat(retryMatch[1]));
                            }

                            // Also check for retryDelay in the error JSON
                            const retryDelayMatch = error.message.match(/"retryDelay":\s*"(\d+)s"/);
                            if (retryDelayMatch) {
                                retryAfterSeconds = parseInt(retryDelayMatch[1], 10);
                            }

                            console.log(`[GEMINI] Rate limited for ${retryAfterSeconds} seconds`);

                            // Build pending request for auto-retry
                            const pendingRequest = {
                                type: input.media ? 'gemini-screenshot' : 'gemini-text',
                                text: input.text || '',
                                imageData: input.media ? input.media.data : null,
                            };

                            // Notify renderer about rate limit with pending request
                            ...
                            shortMsg = `Rate limited (${retryAfterSeconds}s)`;
...
async function startMacOSAudioCapture(geminiSessionRef, vadEnabled = false, vadMode = 'automatic', useGroqForSTT = false) {
    ...
    if (macVADEnabled) {
        ...
        if (macVADMode === 'automatic') {
            ...
            console.log('🎤 [macOS VAD AUTOMATIC] Microphone enabled by default');
        } else {
            macMicrophoneEnabled = false;
            console.log('🔴 [macOS VAD MANUAL] Microphone OFF - Press Cmd+Shift+M to enable');
        }
        ...
        // VAD disabled: enable microphone by default so audio flows
        macMicrophoneEnabled = true;
        console.log('🎤 [macOS VAD DISABLED] Microphone enabled by default - audio will be processed continuously');
    }
    ...
    systemAudioProc.stdout.on('data', data => {
        ...
        while (audioBuffer.length >= CHUNK_SIZE) {
            ...
            if (macVADEnabled && macVADProcessor) {
                ...
                if (!macMicrophoneEnabled) {
                    ...
                    droppedChunkCount++;
                    if (droppedChunkCount === 1 || droppedChunkCount % 100 === 0) {
                        console.warn(
                            `⚠️ [macOS] Audio being dropped - microphone is OFF. Press Cmd+Shift+M to enable. (${droppedChunkCount} chunks dropped)`
                        );
                    }
                    continue;
                }
        ...
        if (audioBuffer.length > maxBufferSize) {
            audioBuffer = audioBuffer.subarray(-maxBufferSize);
        }
    });
...
function setupGeminiIpcHandlers(geminiSessionRef) {
    ...
    ipcMain.handle(
        'initialize-gemini',
        async (event, apiKey, customPrompt, profile = 'interview', language = 'en-US', mode = 'interview', model = 'gemini-2.5-flash') => {
    ...
    ipcMain.handle('send-image-content', async (event, { data, debug, isManual }) => {
        ...
        try {
            ...
            if (currentMode === 'interview' && isManual) {
                ...
                await geminiSessionRef.current.sendRealtimeInput({
                    text: "Based on this screenshot: If you see a CODING PROBLEM (LeetCode, HackerRank, CodeSignal, etc. with a code editor), CRITICAL: DETECT THE EXACT PROGRAMMING LANGUAGE visible in the editor (C++, Java, Python, JavaScript, TypeScript, Go, Rust, etc.) and provide the COMPLETE CODE SOLUTION in THAT EXACT LANGUAGE ONLY. DO NOT default to Python or any other language - use EXACTLY the language shown on screen. Use the EXACT function signature visible (same class name, method name, parameter count/types/names, return type). DO NOT modify the signature or language. DO NOT search online for similar problems. Format: [1-line approach] + [clean code block without comments in the DETECTED LANGUAGE using EXACT signature] + [complexity] + [algorithm explanation with 2-4 brief bullet points so I can explain the approach to the interviewer]. If it's an APTITUDE/MCQ/REASONING question, answer directly and concisely in 2-3 sentences - DO NOT say 'This is a word problem' or 'not a coding question', just give the answer. If it's a regular interview question, answer briefly (2-3 sentences). If it's just code to review, explain what it does. If unclear, describe what you see.",
                });
            } else {
